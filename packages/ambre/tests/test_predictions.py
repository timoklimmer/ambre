...
def test_predict_correct_probability_when_single_antecedent_is_given():
    ...
    bread_consequent_result = database.predict_consequents_records(["butter"])[0]
...
def test_predict_correct_probability_when_multiple_antecedents_are_given():
    ...
    database = get_wikipedia_database_consequent_bread()
    bread_consequent_result = database.predict_consequents_records(["butter", "milk"])[0]
    assert bread_consequent_result["consequent"] == "bread"
...
def test_predict_none_when_antecedent_does_not_exist():
    ...
    database = get_wikipedia_database_consequent_bread()
    assert database.predict_consequents_records(["i_dont_exist"])[0]["probability"] is None
    assert database.predict_consequents_records(["butter", "i_dont_exist"])[0]["probability"] is None
...
def test_predict_skips_unknown_antecedents_when_desired():
    ...
    database = get_wikipedia_database_consequent_bread()
    assert (
        database.predict_consequents_records(["butter", "coke"], skip_unknown_antecedents=True)[0]["probability"] == 0.5
    )
    assert (
        database.predict_consequents_records(["i_dont_exist"], skip_unknown_antecedents=True)[0]["probability"] is None
    )
...
def test_predict_priors_when_no_antecedents():
    ...
    database = get_wikipedia_database_consequent_bread()
    bread_consequent_result = database.predict_consequents_records()[0]
    assert bread_consequent_result["consequent"] == "bread"
...
def test_predict_empty_list_when_no_consequents():
    ...
    assert database.predict_consequents_records() == []
...
def test_predict_specified_consequents_only():
    ...
    assert len(database.predict_consequents_records(consequents=["bread"])) == 1
...
def test_predict_throws_when_specified_consequent_does_not_exist():
    ...
    with pytest.raises(
        ...
        database = get_wikipedia_database_consequent_bread_and_milk()
        assert database.predict_consequents_records(consequents=["pasta"])
